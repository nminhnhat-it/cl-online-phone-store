...
export default {
  ...
  computed: {
    ...
    getprepareOrders() {
      ...
      if (orders) {
        var news = orders.filter(order => order.od_status == 'prepare');
        return news;
      }
    },
  },
...
<template>
  <div class="content">
    ...
    <dataTable :back="'admin.order.prepares'" v-if="this.route[3] == null" :route="this.route" @cancel:item="cancelOrder" @approve:item="approveOrder" @ship:item="shipOrder" @complete:item="completeOrder" :dataArr="getprepareOrders" />
